...
def start_auth_flow(state: str | None = None):
    """Create an OAuth flow and return the authorization URL, state and flow.

    Parameters
    ----------
    state:
        Optional state token to include in the authorization request. If not
        provided, a random token is generated by ``authorization_url``.
    """
    ...
    auth_url, returned_state = flow.authorization_url(prompt="consent", state=state)
    return auth_url, returned_state, flow
